...
def create_files():
    ...
    with open('/tmp/steamlink-launcher.sh', 'w') as outfile:
        outfile.write('#!/bin/sh\n'
                      ...
                      'if [ -f "openvt" ]\n'
                      'then sudo openvt -c 7 -s -f clear\n'
                      'sudo su - osmc -c "nohup openvt -c 7 -f -s steamlink >/dev/null 2>&1 &" &\n'
                      'sudo systemctl stop mediacenter &\n'
                      'else systemctl stop kodi && /storage/steamlink; fi\n'
                      '}\n'
                      'install_steam () {\n'
                      ...
                      'then chown osmc /storage; fi\n'
                      'if [ $(cat /etc/os-release | grep OSMC | wc - l) -eq 1 ]\n'
                      'then sudo apt-get install curl gnupg libc6 xz-utils -y\n'
                      ...
                      'rm steamlink.tar.gz\n'
                      'if [ $(cat /etc/os-release | grep OSMC | wc - l) -eq 1 ]\n'
                      'then chown osmc /storage/steamlink\n'
                      'chmod +x osmc /storage/steamlink\n'
                      'usermod -a -G input,plugdev osmc\n'
                      'else usermod -a -G input,plugdev root; fi\n'
                      'start_steamlink\n'
                      '}\n'
